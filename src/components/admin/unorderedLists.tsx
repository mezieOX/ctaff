    ...
    <Flex flexDir="column" mt="2rem">
      ...
      {data.length > 0? (                  
          <UnorderedList>
            {data?.map((item: any, i) => (
                type == "lang"?
                  <ListItem key={i}>{item.languageType} - {item.fluency || 0}%</ListItem>: 
                type == "skill"? 
                  <ListItem key={i}>{item.skillType} - {item.skillLevel || 0}%</ListItem>:
                type == "availDays"? 
                  <ListItem key={i}>{item.day} - ({item.am && "am"}{item.am && item.pm && " & "}{item.pm && "pm"})</ListItem>:
